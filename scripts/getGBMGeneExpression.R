...
exp_eSet <- ExpressionSet(
  assayData = as.matrix(assay_exp), phenoData = AnnotatedDataFrame(phen_exp),
...
ctrl <- read.delim(file.path(download_dir, "HK_genes.txt"), stringsAsFactors = FALSE, header = FALSE) # Negative control genes
...
for (i in 1:nrow(rep_cIdx)) {
  ...
  if (length(GSM) == 2) {
    ...
    (rep_cIdx[count, 1] <- which(rownames(YY) == GSM))
  }
...
exp_eSet_ruv <- ExpressionSet(
  assayData = as.matrix(assay_exp_ruv), phenoData = AnnotatedDataFrame(phen_exp),
  featureData = AnnotatedDataFrame(feat_exp),
...
exp_eSet_probe <- ExpressionSet(
  assayData = as.matrix(assay_exp_probe), phenoData = AnnotatedDataFrame(phen_exp),
...
saveRDS(expression_SE, file.path(processed_dir, "expression_SE.rds"))
saveRDS(expression_ruv_SE, file.path(processed_dir, "expression_ruv_SE.rds"))
saveRDS(expression_probe_SE, file.path(processed_dir, "expression_probe_SE.rds"))
saveRDS(phen_exp, file.path(processed_dir, "phen_exp.rds"))


unlink(file.path(processed_dir, "GSE152160_RAW"), recursive = TRUE)
