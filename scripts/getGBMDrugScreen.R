...
phen_exp <- readRDS(file.path(processed_dir, "phen_exp.rds"))
phen_cnv <- readRDS(file.path(processed_dir, "phen_cnv.rds"))
phen_mutation <- readRDS(file.path(processed_dir, "phen_mutation.rds"))
phen_methyl <- readRDS(file.path(processed_dir, "phen_methyl.rds"))
...
screen_objects <- function(screen, cell_obj, drug_obj, drug_with_ids_gbm, d_c_l) {
  ...
  screen[, 3:ncol(screen)] <- apply(screen[, 3:ncol(screen)], 2, function(x) {
    x * 100
  })
  ...
  for (i in 1:length(diff_scr)) {
    ...
    if (cid %in% scr_cell_obj$cellid) {
      ...
      scr_cell_obj[nrow(scr_cell_obj) + 1, ] <- c(NA, NA, NA, diff_scr[i], rep(NA, length(colnames(scr_cell_obj)) - 4))
    }
  }
  ...
  scr_cur_cell <- data.frame(
    unique.cellid = rownames(scr_cell_obj),
    GBM.cellid = rownames(scr_cell_obj),
    row.names = rownames(scr_cell_obj)
  )

  ########## Curation drug dataframe
  scr_cur_drug <- data.frame(
    drugid = scr_drugs$drugid,
    GBM.drugid = scr_drugs$GBM.drugid,
    row.names = scr_drugs$drugid
  )

  ########## Curation tissue dataframe
  scr_cur_tissue <- data.frame(data.frame(
    tissueid = rep("GBM", nrow(scr_cell_obj)),
  ...
  return(list(
    "sen_info" = sen_info_scr, "sen_raw" = sen_raw_scr, "sen_profile" = sen_profile_scr,
...
drug_with_ids_gbm <- read.csv(file.path(download_dir, "drugs_with_ids.csv"), stringsAsFactors = FALSE, na.strings = "") # this is the drug with ids file downloaded form pachy annotation repo
...
# ======================== Drug object ========================
drugs <- read.xlsx(file.path(download_dir, "mmc3.xlsx"), rowNames = TRUE, startRow = 2)
...
# ======================== Sensitivity data ========================
# Published AUC info
drug_cell <- read.delim(file.path(download_dir, "HGCC_drug_response_AUC.txt"), header = T, sep = "\t", stringsAsFactors = FALSE)
...
screen2 <- read.delim(file.path(download_dir, "Screen2-drugData.txt"), stringsAsFactors = FALSE) # Drug_dose_scr2 response data
...
screen3 <- read.delim(file.path(download_dir, "Screen3-drugData.txt"), stringsAsFactors = FALSE) # Drug_dose_scr2 response data
...
saveRDS(scr2_objects, file.path(processed_dir, "scr2_objects.rds"))
saveRDS(scr3_objects, file.path(processed_dir, "scr3_objects.rds"))
